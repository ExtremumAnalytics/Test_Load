...
def upload_to_blob(file_content, session, blob_service_client, container_name):
    ...
    if 'login_pin' not in session:
        return "Error: 'login_pin' not found in session."
    try:
        folder_name = "pharma/" + str(session['login_pin'])
...
def update_bar_chart_from_blob(session, blob_service_client, container_name):
    ...
    try:
        ...
        blob_list = blob_service_client.get_container_client(container_name).list_blobs(name_starts_with='pharma/'+folder_name)
...
def update_when_file_delete():
    ...
    all_blobs = blob_service_client.get_container_client(container_name).list_blobs(name_starts_with='pharma/' + folder_name_azure)
    # print(all_blobs)
    ...
    for blob in all_blobs_list:
        ...
        if file_name.endswith('.csv') or file_name.endswith('.CSV') or file_name.endswith('.txt'):
            session['progress_files'].append(file_name)
            socketio.emit('success', session['progress_files'])

    blob_list = [blob for blob in all_blobs_list if not (blob.name.endswith('.csv') or blob.name.endswith('.CSV') or blob.name.endswith('.txt'))]
    # print(blob_list)
    ...
    search_client = SearchClient(
        ...
        index_name='pharma-' + folder_name_azure,
    ...
    try:
        if blob_list_length != 0:
            for blob in result_loop:
                ...
                if file_name.endswith('.pdf') or file_name.endswith('.PDF') or file_name.endswith('.mp3') or file_name.endswith('.txt'):
                ...
                if '.mp3' in file_name and len(chunks) != 0:
                    ...
                    with open(temp_pdf_path, "rb") as file:
                        content = file.read()
                    blob_name = f"pharma/{folder_name_azure}/{file_name}"
                    blob_client = blob_service_client.get_blob_client(container=container_name, blob=blob_name)
...
def get_conversation_chain(vectorstore, source):
    ...
    if source == 'myFiles':
        ...
        template = f"""{{context}}Use the following pieces of context from the provided files only. Do not use any information from the internet to answer the question at the end. 
        ...
        context = "Use the following pieces of context from the internet or web to answer the question at the end."
        template = f"""{{context}}
                    If you don't know the answer,
                    just say that you don't know, don't try to make up an answer. Use three sentences maximum. Keep the answer as concise as possible.
                    Question: {{question}}
    ...
    # template = f"""{{context}} If you don't know the answer,
    #         just say that you don't know, don't try to make up an answer. Use three sentences maximum. Keep the answer as concise as possible.
    #         Question: {{question}}
    #         Helpful Answer:"""
    ...
    if request.method == 'POST':
        ...
        #     print('database url n all.......', db_url, username, password)
            ...
            if not request.form.get('Source_URL', ''):
                ...
                # Upload the Source_URL as a text file
            Source_URL = request.form.get('Source_URL', '')
            urls = Source_URL.split(',')
            print(type(urls))
            for url in urls:
                keyword = url.strip().split('/')[-1] or url.strip().split('/')[-2]  # Extract keyword from the URL
                filename = f"{keyword}_url.txt"
                temp_file = BytesIO(url.encode('utf-8'))
                temp_file.filename = filename
                temp_file.content_type = 'text/plain'
                upload_to_blob(temp_file, session, blob_service_client, container_name)
                print(f"URL Found---->{url}, Saved as {filename}")
            # fake_file = BytesIO(Source_URL.encode('utf-8'))
            # fake_file.filename = 'source_url.txt'
            # fake_file.content_type = 'text/plain'
            # upload_to_blob(fake_file, session, blob_service_client, container_name)
            # print("Source_URL Found---->", Source_URL)
...
def run_query(data):
    ...
    folder_name_azure = 'pharma/' + str(session['login_pin'])
...
def handle_ask_question(data):
    ...
    try:
        ...
        conversation = get_conversation_chain(vector_store, Source)
...
def handle_get_dropdown_data():
    ...
    try:
        ...
        for blob in blob_list:
            ...
            id_from_name = blob.name.split('/')[1]
            ...
            if user and user_role:
                if 'Doctors_List.csv' in blob.name:
                    doctors_file = blob.name
                elif 'Products_List.csv' in blob.name:
                    products_file = blob.name

        if not doctors_file or not products_file:
            emit('dropdown_data', {'error': 'Required files not found in the container'})
            return

        # Read CSV files from Azure Blob Storage
        doctors_df = pd.read_csv(get_blob_content(doctors_file))
        products_df = pd.read_csv(get_blob_content(products_file))

        # Filter based on user ID
        filtered_doctors = doctors_df[doctors_df['user_id'] == user_id]
        filtered_products = products_df[products_df['user_id'] == user_id]

        # Convert to dictionaries
        doctors_list = filtered_doctors.to_dict('records')
        ...
        emit('dropdown_data', {'error': str(e)})
...
def delete_files():
    try:
        ...
        for file_name in file_names:
            blobs = container_client.list_blobs(name_starts_with='pharma/'+folder_name)
    ...
    try:
        ...
        for result in results:
            ...
            if document and document not in unique_documents:
                VecTor_liSt.append(document)
                unique_documents.add(document)
        blobs = container_client.list_blobs(name_starts_with="pharma/"+index_name)
        print("Blob List",blobs)
...
def handle_request_admin_vault_data():
    ...
    for blob in blobs:
        if 'pharma' in blob.name and 'draft' not in blob.name:
            # Extract the ID from the blob name (up to the first slash)
            id_from_name = blob.name.split('/')[1]
...
def handle_upload_to_user_vault(data):
    try:
        ...
        for file in file_urls:
            file_url = file['url']
            blob_name = f"pharma/{user_folder}/{file_url.split('/')[-1]}"
...
def upload_draft_to_blob(file_content, session, blob_service_client, container_name):
    ...
    try:
        ...
        blob_name = f"pharma/{folder_name}/draft/{file_content.filename}"
        blob_client = blob_service_client.get_blob_client(container=container_name, blob=blob_name)
...
def upload_draft():
    try:
        ...
        # Notify all users about the new draft
        socketio.emit('new_draft', {'message': f'A new draft "{file.filename}" has been uploaded by {session["user_name"]}'})
...
def draft_table_update():
    try:
        ...
        container_client = blob_service_client.get_container_client(container_name)
        blob_list = container_client.list_blobs(name_starts_with='pharma/'+str(session.get('login_pin')))
...
# Fetch guest users
@socketio.on('get_guest_users')
def handle_get_guest_users():
    # Define the role_id_mapping
    role_id_mapping = {'Admin': 1, 'Guest': 2, 'ML Engine': 3}

    # Use the role_id_mapping to find the corresponding role_id for 'Guest'
    role_id = role_id_mapping.get('Guest')

    if not role_id:
        emit('guest_users_data', {'error': 'Role ID for Guest not found'})
        return

    # Query the database to get users with the role_id for 'Guest' and status 'Active'
    guest_users = UserDetails.query.filter_by(role_id=role_id, status='Active').all()

    user_list = [{'id': user.login_pin, 'first_name': user.first_name.title(), 'last_name': user.last_name.title(), 'email_id': user.email_id}
                 for user in guest_users]
    emit('guest_users_data', {'users': user_list})

@socketio.on('send_notice')
def handle_send_notice(data):
    user_id = data.get('userID')
    message = data.get('notice')

    if not user_id or not message:
        emit('notice_status', {'error': 'User ID and message are required'})
        return
    ...
    if user_id == 'All':
        ...
        if not role_id:
            emit('notice_status', {'error': 'Role ID for Guest not found'})
            return

        guests = UserDetails.query.filter_by(role_id=role_id, status='Active').all()

        for guest in guests:
            # new_notice = Notice(user_id=guest.id, message=message)
            # db.session.add(new_notice)
            # db.session.commit()
            socketio.emit('new_notice', {'userID': guest.login_pin, 'message': message}, room=str(guest.login_pin))

    else:
        # Store the notice in the database
        # new_notice = Notice(user_id=user_id, message=message)
        # db.session.add(new_notice)
        # db.session.commit()
        # Emit the message to the specific user via Socket.IO
        socketio.emit('new_notice', {'userID': user_id, 'message': message}, room=user_id)

    emit('notice_status', {'success': True, 'message': 'Notice sent successfully'})
...
# @socketio.on('get_notices')
# def handle_get_notices(data):
#     user_id = data.get('userID')
#     if not user_id:
#         emit('notices_data', {'error': 'User ID is required'})
#         return
#
#     # Retrieve notices for the user from the database
#     user_notices = Notice.query.filter_by(user_id=user_id).all()
#     notice_list = [{'message': notice.message, 'created_date': notice.created_date} for notice in user_notices]
#
#     emit('notices_data', {'notices': notice_list})
